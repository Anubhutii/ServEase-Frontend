...
const UserChatbox: React.FC<UserChatboxProps> = ({ bookingId, onClose, isPage: _isPage }) => {
  const { user } = useAuth();
  ...
  const handleSendOffer = async () => {
    ...
    try {
      ...
      setOfferPrice('');
      fetchNegotiation();
      message.success("Price offer submitted to provider");
    } catch (error) {
      console.error(error);
      message.error("Failed to submit price offer");
    }
  };
  ...
  const providerName = negotiation?.provider?.firstName
    ? `${negotiation.provider.firstName} ${negotiation.provider.lastName || ""}`
    : negotiation?.provider?.name || "Service Partner";

  return (
    <div className={`flex flex-col h-[520px] w-full select-text ${isDark ? "bg-slate-900 text-slate-100" : "bg-white text-slate-800"}`}>
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded-full text-[9px] font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 shrink-0">
                <ShieldCheck className="w-2.5 h-2.5" />
                <span>Partner</span>
              </span>
            </div>
            <p className="text-[11px] text-slate-400 m-0 truncate">
              {negotiation?.booking?.job?.title || "Direct Booking Negotiation"}
            </p>
          </div>
        </div>
        ...
        <>
          ...
          <div className={`flex-1 overflow-y-auto p-4 space-y-3 ${
            ...
            <div className="text-center py-2">
              <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-[10px] font-semibold bg-blue-500/10 text-blue-600 dark:text-blue-400">
                <Sparkles className="w-3 h-3" />
                <span>Encrypted live negotiation &amp; support</span>
              </div>
            </div>
            ...
            {(!negotiation?.messages || negotiation.messages.length === 0) && (
              <div className="text-center py-10 text-xs text-slate-400">
                <p className="m-0">No messages yet. Send a message to negotiate or confirm details!</p>
              </div>
            )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-semibold">
                    {isOfferByMe ? "Your proposed price:" : "Provider's offer:"}
                  </span>
                </div>
          ...
          <div className={`p-3 border-t ${
            ...
            {negotiation?.status === 'active' || !negotiation?.status ? (
              ...
              <div className="text-center text-xs text-slate-400 italic py-1">
                This booking negotiation has been finalized.
              </div>
            )}
          </div>
        </>
